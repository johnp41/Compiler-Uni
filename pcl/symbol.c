...
static SymbolEntry * newEntry (const char * name)
{
 ...
 //   printf("%s\n",name);
    //printf("exw vgalei erroro na ksanavalw");
...
static unsigned int fixOffset (SymbolEntry * args)
{
    int counter = 0;
    while (args != NULL) {
//        args->u.eParameter.offset = counter;
//        counter++;
//        args = args->u.eParameter.next;
          args->u.eParameter.offset = currentScope->negOffset;
          currentScope->negOffset++;
          args = args->u.eParameter.next;
    }
   // return counter;
      return currentScope->negOffset -1;
...
SymbolEntry * newTemporary (Type type)
{
    ...
    if (e != NULL) {
        ...
        e->u.eVariable.type = type;
        type->refCount++;
      //  printf( "current scope offset was : %d\n",currentScope->negOffset  );
        currentScope->negOffset -= sizeOfType(type);
...
bool isIArray(Type t){
    if(t->kind == TYPE_IARRAY) return true ;
    else return false ;
}
...
/*
bool isCompatibgsle(Type t1,Type t2){
    
    internal("my mistake here");
    return false ;
}
*/

bool isComp(SymbolEntry* t,Type t2){
    //if(t == NULL || t2 == NULL) cout << "edw tin ekana" << ens
  //  printf( "eimai sto is comp \n") ;
    if(t2 == NULL) internal("SOMETHING WRONG HAPPENED"); 
    if(t->u.eParameter.mode == PASS_BY_VALUE) return myequalType(t->u.eParameter.type,t2,0);
    /*if(t->u.eParameter.mode == PASS_BY_REFERENCE  && (t->u.eParameter.type->refType != NULL) ){ printf( "pass by ref\n") ;  return myequalType((t->u.eParameter.type->refType),t2);}
   */
   
      //I dont think we need it like that
    if(t->u.eParameter.mode == PASS_BY_REFERENCE   ){   
        return myequalType(typePointer((t->u.eParameter.type)),typePointer(t2),1);
        }
  //  if(t->u.eParameter.mode == PASS_BY_REFERENCE   ){   return myequalType((t->u.eParameter.type),t2,0);}
    else error("wrong pass by reference");
    return false ;
}

bool myequalType(Type type1,Type type2,int n){

      
        if (type1->kind == TYPE_REAL && type2->kind == TYPE_INTEGER && n == 0) return true ;
        if (type1->kind != type2->kind) return false ;
        switch (type1->kind) {

            case TYPE_ARRAY:
            //gia na exei size einai kai o allos tpye array
                //if(type2->kind == TYPE_IARRAY) return myequalType(type1->refType,type2->refType);
                if ( type1->size != type2->size) return false;
                  

            case TYPE_IARRAY:
                return myequalType(type1->refType, type2->refType,1);

            case TYPE_POINTER:
                if( (type2->refType->kind == TYPE_VOID) ) return true ;   
                if((type1->refType->kind == TYPE_IARRAY) && (type2->refType->kind == TYPE_ARRAY)  ) return myequalType(type1->refType->refType , type2->refType->refType,1);
               //  return true;    
               
                else  false ;//return myequalType(type1->refType , type2->refType,1);
        }
    
        //eprepe na kanw ta parapanw gia type 2
     
    return true;
}

//TO-do : elenksw pws elenxetai h isothta---exw dup gtiagmena stract --ti 8a vgalei h isotita
bool  check_if_numer(Type type1,Type type2){

    if (! (type1 == typeInteger || type1 == typeReal) ) return false ;//error("must be numerical");
    if (! (type2 == typeInteger|| type2 == typeReal) ) return false ; //error("must be numerical");

    return true ;

}

bool check_whole_type (Type type1){

    if(!(type1->kind == TYPE_POINTER )) return false ;
    type1=type1->refType;
    if(type1 == typeChar) return true ;
    if(type1 == typeBoolean) return true ;
    if(type1 == typeReal) return true ;
    if(type1 == typeInteger) return true ;
    else return false;
}

bool check_new_ex(Type t1,Type t2){
    if(!((t1->kind == TYPE_POINTER) && (t1->refType->kind == TYPE_IARRAY) )) return false ;
    if(!(t2->kind == TYPE_INTEGER)) return false ;
    else return true ;
}

bool check_disp_arr(Type t1){
    if(!((t1->kind == TYPE_POINTER) && (t1->refType->kind == TYPE_IARRAY) )) return false ;
   // if(!(t2->kind == TYPE_INTEGER)) return false ;
    else return true ;
}
